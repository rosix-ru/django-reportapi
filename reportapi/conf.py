...
REPORTAPI_ROOT = getattr(settings, 'REPORTAPI_ROOT',
                         '%sreports/' % settings.MEDIA_ROOT)
REPORTAPI_URL = getattr(settings, 'REPORTAPI_URL',
                        '%sreports/' % settings.MEDIA_URL)
REPORTAPI_ENABLE_THREADS = getattr(settings, 'REPORTAPI_ENABLE_THREADS', False)
REPORTAPI_FILES_UNIDECODE = getattr(settings, 'REPORTAPI_FILES_UNIDECODE',
                                    False)
REPORTAPI_LANGUAGES = getattr(settings, 'REPORTAPI_LANGUAGES', ['en', 'ru'])
REPORTAPI_UPLOADCODE_LENGTH = getattr(settings, 'REPORTAPI_UPLOADCODE_LENGTH',
                                      12)
...
REPORTAPI_MAXSIZE_XML = getattr(settings, 'REPORTAPI_MAXSIZE_XML',
                                1048576 * 50)
...
class Header(object):
    ...
    dynamic_spacing = getattr(settings, 'REPORTAPI_HEADER_DYNAMIC_SPACING',
                              'false')
...
class Footer(object):
    ...
    border_top = getattr(settings, 'REPORTAPI_FOOTER_BORDER_TOP',
...
class Page(object):
    ...
    print_orientation = getattr(settings, 'REPORTAPI_PAGE_PRINT_ORIENTATION',
                                'portrait')
    footnote_max_height = getattr(settings,
                                  'REPORTAPI_PAGE_FOOTNOTE_MAX_HEIGHT', '0cm')
...
SERVER_TZ_OFFSET = int(
    SERVER_TZ.utcoffset(datetime.now()).total_seconds() / 60 * -1
)
