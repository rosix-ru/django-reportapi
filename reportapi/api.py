...
from quickapi.utils.doc import apidoc_lazy, string_lazy, RETURN_BOOLEAN_SUCCESS
...
def result(request, document, old=False):
    ...
    result = {
        ...
        'convert_error': deep_from_dict(document.details, 'convert.error',
                                        update=False),
...
@api_required
@login_required
def API_document_create(request, section, name, filters=None, force=False,
                        fake=False, **kwargs):
    """
    ...
    expired = timezone.now() - timezone.timedelta(
        seconds=report.expiration_time
    )
    ...
    if proc_documents:
        ...
        # Новый отчёт
        document = Document.objects.new(request=request, user=user, code=code,
                                        register=register)
        document.description = report.get_description_from_filters(
            filters, request=request
        )
        document.details = report.get_details(document=document,
...
information about an already running process.*"""
    ...
    5. "fake"    - %s.
        """, (
            ...
            _('for finished documents simulates the creation of new '
              '(optional)')
        )
    ),
    data=string_lazy(
...
@api_required
@login_required
def API_object_search(request, section, name, filter_name, query=None, page=1,
                      **kwargs):
    """
...
API_object_search.__doc__ = apidoc_lazy(
    header=_(
        """*Searches for filling the object filter an instance of the related
model.*"""
    ),
    params=string_lazy(
        """
...
```
        """, (
            _('The serialized object to the page of Paginator'),
            _('First object'),
            _('Second object')
        )
    ),
    footer=_(
        """
*This is the standard output, which can be overridden separately
 for each report.*
        """
    )
)
